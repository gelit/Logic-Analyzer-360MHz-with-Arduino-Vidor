/* 27 fév 2023 from EmptySketch
Pin Used : D0		    oTshift
           D1-D7		7 input	
           D8-D14	  7 output	
           A0       20 fév : A0=0 --> Fech Divider Enable / A==1 --> Fech=360 MHz
           A1		    Byte Available		
           A2-A4		Frequency cmd	
           A5-A6		Trigger	cmd
*/
...
// DON'T USE pin 13-14 used by FPGA
  ...
  pinMode(A0,OUTPUT); digitalWrite(A0,0); // 20 fév : A0=0 --> Fech Divider Enable / A==1 --> Fech=360 MHz
  ...
  if (Serial.available() > 0) {
    ...
    switch (Y) {
     ...
     case 'S' : if            (!digitalRead(A5) && !digitalRead(A6)) {  // Toggle (Running / Stopped)
      ...
      break; 
                                                                                                           // UP
      case 'U' : if (Div==1) {Div--; digitalWrite(A0,1); Serial.println("360 MHz");} //2e0 --> 360 MHz
                 else if (Div>1) {     
                   digitalWrite(A3,0); delayMicroseconds(50);                                              // !Reset 
                   Div--; 
                   if (Div==1) {} else {for (N=1; N<=Div; N++) {X++; one();}}                                          
                   digitalWrite(A2,1); delayMicroseconds(10); digitalWrite(A2,0); delayMicroseconds(10);   // Latch
                   digitalWrite(A3,1); 
                 }                                   
      break; 
                                                                                                        // DOWN
      case 'D' : if (Div==0) {Div++; digitalWrite(A0,0); Serial.println("no 360 MHz");}
                 else {
                   digitalWrite(A3,0); delayMicroseconds(50);
                   Div++; if (Div>20) {Div=20;} 
                   for (N=1; N<=Div-1; N++) {X++; one();} 
                   digitalWrite(A2,1); delayMicroseconds(10); digitalWrite(A2,0); delayMicroseconds(10);
                   digitalWrite(A3,1);
                 }                   
      break; 
