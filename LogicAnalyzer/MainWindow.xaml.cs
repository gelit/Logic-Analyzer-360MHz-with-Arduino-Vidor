...
namespace LogicAnalyzer
{
    public partial class MainWindow : Window
    {
        ...
        public MainWindow()
        {
            ...
            timer.Start();
            USB = false; Curso = 1; Cu1V = 0; Cu2V = 0; Stop = false; UpDownV = 1; Div = 5;
            DisplayTim();
            PrintLargeTime();
        }
        ...
        private void DisplayTim()
        {
            ...
            {
                if (Cu1V > Cu2V) { v = (Cu1V - Cu2V) * Tech; }
                else {             v = (Cu2V - Cu1V) * Tech; }

                if (v < 1000) {                    string s = v.ToString("0"); TimeV.Text = s + " nanos"; }
                else if (v < 1000000) { v /= 1000; string s = v.ToString(".00"); TimeV.Text = s + " micros"; }
                else { v /= 1000000;               string s = v.ToString(".00"); TimeV.Text = s + " millis"; }
            }
        }
...

        {
            ...
            if (e.Key == Key.Down && Div < 20)  { SerialTx(Convert.ToByte('D')); Div++;  Tech = 2 * Tech;  PrintLargeTime(); Refresh('D'); }  
            if (e.Key == Key.Up   && Div >  0) { SerialTx(Convert.ToByte('U')); Div--;  Tech = Tech / 2;  PrintLargeTime(); Refresh('U');  }

            if (e.Key == Key.D1) { SerialTx(Convert.ToByte('1')); }  // Test 20 fév
